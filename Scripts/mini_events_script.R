...
mini_event <- "Hero Xp Event"

duration_mini_event <- "2" #Please input the duration of the mini event in days 

event_start_date <- "2017-07-01"
week_before_start <- "2017-06-11"
...
ggplot(arpdau, aes(date,arpdau)) + geom_line(size = 1, colour = "green4") + geom_smooth()+
  ...
  annotate("text",x = as.Date(event_start_date)+3, y=min(arpdau$arpdau)-0.5*(min(arpdau$arpdau)),label = mini_event, colour = "dodgerblue4")+
  ...
  annotate("text",x = as.Date(week_before_start) + 3, y=min(arpdau$arpdau)-0.5*(min(arpdau$arpdau)),label = "Week Before", colour = "dodgerblue4")+
